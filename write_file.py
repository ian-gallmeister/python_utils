...
def write_file( name, text, encoding='utf-8' ):
  version = sys.version_info[0]
  if version == 2 and isinstance(text, str):
    open( name, 'wb' ).write( text ) #Is bytes already
  elif version == 2 and isinstance(text, unicode):
    open( name, 'wb' ).write( text.encode(encoding) ) #Turn to bytes
  elif version != 2:
    open( name, 'w', encoding=encoding ).write( text )
  return True
